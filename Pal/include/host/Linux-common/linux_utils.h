...
/* Runs a command in a subprocess (fork + execve); reads its stdout */
int run_command(const char* pathname, const char** argv, char* buf, size_t buf_size,
                size_t* out_len);

/* Callback for `parse_proc_maps` */
typedef int (*proc_maps_callback_t)(uintptr_t start, uintptr_t end, size_t offset, const char* name,
                                    void* arg);
