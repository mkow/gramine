...
static int build_envs(const char** orig_envp, bool propagate, const char*** out_envp) {
    ...
    toml_table_t* toml_loader = toml_table_in(g_pal_common_state.manifest_root, "loader");
    if (!toml_loader)
...
static void configure_logging(void) {
    ...
    ret = toml_string_in(g_pal_common_state.manifest_root, "loader.debug_type", &debug_type);
    if (ret < 0)
    ...
    char* log_file = NULL;
    ret = toml_string_in(g_pal_common_state.manifest_root, "loader.log_file", &log_file);
    if (ret < 0)
...
noreturn void pal_main(uint64_t instance_id,       /* current instance id */
    ...
    assert(g_pal_common_state.manifest_root);
    assert(g_pal_common_state.alloc_align && IS_POWER_OF_2(g_pal_common_state.alloc_align));

    configure_logging();

    char* dummy_exec_str = NULL;
    ret = toml_string_in(g_pal_common_state.manifest_root, "loader.exec", &dummy_exec_str);
    ...
    ret = toml_string_in(g_pal_common_state.manifest_root, "loader.argv0_override",
                         &argv0_override);
    if (ret < 0)
    ...
    if (!use_cmdline_argv) {
        ...
        ret = toml_string_in(g_pal_common_state.manifest_root, "loader.argv_src_file",
                             &argv_src_file);
        if (ret < 0)
    ...
    ret = toml_string_in(g_pal_common_state.manifest_root, "loader.env_src_file", &env_src_file);
    if (ret < 0)
    ...
    ret = toml_string_in(g_pal_common_state.manifest_root, "loader.entrypoint", &entrypoint_name);
    if (ret < 0)
        INIT_FAIL_MANIFEST(PAL_ERROR_INVAL, "Cannot parse 'loader.entrypoint'");

    if (!entrypoint_name) {
        ret = toml_string_in(g_pal_common_state.manifest_root, "loader.preload", &entrypoint_name);
        if (ret < 0)
    ...
    g_pal_control.manifest_root   = g_pal_common_state.manifest_root;
    ...
    g_pal_control.alloc_align = g_pal_common_state.alloc_align;
