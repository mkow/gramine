...
int sgx_get_seal_key(uint16_t key_policy, sgx_key_128bit_t* out_seal_key) {
    ...
    memcpy(&key_request.cpu_svn, &g_pal_state.enclave_info.cpu_svn, sizeof(sgx_cpu_svn_t));
    memcpy(&key_request.isv_svn, &g_pal_state.enclave_info.isv_svn, sizeof(sgx_isv_svn_t));
    memcpy(&key_request.config_svn, &g_pal_state.enclave_info.config_svn, sizeof(sgx_config_svn_t));
...
int init_enclave(void) {
    ...
    memcpy(&g_pal_state.enclave_info, &report.body, sizeof(g_pal_state.enclave_info));
    return 0;
}
...
int _DkStreamReportRequest(PAL_HANDLE stream, sgx_report_data_t* my_report_data,
    ...
    memcpy(&target_info.mr_enclave, &g_pal_state.enclave_info.mr_enclave, sizeof(sgx_measurement_t));
    memcpy(&target_info.attributes, &g_pal_state.enclave_info.attributes, sizeof(sgx_attributes_t));
