...
static int parsed_ranges_callback(uintptr_t start, uintptr_t end, size_t offset, const char* name,
                                    void* arg) {
    __UNUSED(offset);
    ...
    if (name && strcmp(name, "[vdso]") == 0) {
        ranges->vdso_start = start;
        ranges->vdso_end = end;
    }

    if (name && strcmp(name, "[vvar]") == 0) {
        ranges->vvar_start = start;
        ranges->vvar_end = end;
    }
...
int get_vdso_and_vvar_ranges(uintptr_t* vdso_start, uintptr_t* vdso_end, uintptr_t* vvar_start,
    ...
    int ret = parse_proc_maps(&parsed_ranges_callback, &ranges);
    if (ret < 0)
