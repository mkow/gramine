...
unsigned long _DkMemoryQuota(void) {
    if (g_pal_state.memory_quota == (unsigned long)-1)
        return 0;

    if (g_pal_state.memory_quota)
        return g_pal_state.memory_quota;

    unsigned long quota = 0;
    if (read_proc_meminfo("MemTotal", &quota) < 0) {
        g_pal_state.memory_quota = (unsigned long)-1;
        return 0;
    }

    return (g_pal_state.memory_quota = quota * 1024);
}
